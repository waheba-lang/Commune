...
const AddComplaint = () => {
    ...
    const categories = [
        ...
        { id: 'electricity', label: '├ëlectricit├®' },
        { id: 'water', label: 'Eau' },
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (err.response?.status === 422) {
                ...
                setError('├ëchec de l\'envoi de la r├®clamation. Veuillez v├®rifier les champs.');
            }
            console.error(err);
        } finally {
            setLoading(false);
    ...
    return (
        <div className="container" style={{ padding: '3rem 0' }}>
            <div style={{ maxWidth: '800px', margin: '0 auto' }}>
                <h1 style={{ fontSize: '2rem', fontWeight: 'bold', marginBottom: '0.5rem', color: '#1e3c72' }}>D├®poser une r├®clamation</h1>
                <p style={{ color: '#ff8c42', marginBottom: '2.5rem' }}>Fournissez les d├®tails du probl├¿me pour une intervention rapide.</p>
                ...
                <div className="card" style={{ padding: '2.5rem' }}>
                    <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                        ...
                        <div style={{ ...inputGroupStyle, gridColumn: 'span 2' }}>
                            <label style={{ fontSize: '0.875rem', fontWeight: '600' }}>Titre de la r├®clamation</label>
                            <div style={{ position: 'relative' }}>
                                <Type size={18} style={iconStyle} />
                                <input name="title" value={formData.title} onChange={handleChange} required placeholder="Ex: Lampadaire d├®fectueux" style={inputStyle} />
                            </div>
                        ...
                        {/* Category */}
                        <div style={inputGroupStyle}>
                            <label style={{ fontSize: '0.875rem', fontWeight: '600' }}>Cat├®gorie</label>
                            <div style={{ position: 'relative' }}>
                        ...
                        <div style={{ ...inputGroupStyle, gridColumn: 'span 2' }}>
                            <label style={{ fontSize: '0.875rem', fontWeight: '600' }}>Description d├®taill├®e</label>
                            <div style={{ position: 'relative' }}>
                                <AlignLeft size={18} style={{ ...iconStyle, top: '1.2rem' }} />
                                <textarea name="description" value={formData.description} onChange={handleChange} required placeholder="D├®crivez le probl├¿me en d├®tail..." style={{ ...inputStyle, height: '120px', paddingLeft: '2.5rem', resize: 'none', paddingTop: '0.75rem' }} />
                            </div>
                        ...
                        <button type="submit" disabled={loading} className="btn btn-primary" style={{ gridColumn: 'span 2', padding: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', fontSize: '1rem', marginTop: '1rem', backgroundColor: '#1e3c72' }}>
                            <Send size={20} />
                            {loading ? 'Envoi en cours...' : 'Envoyer la r├®clamation'}
                        </button>
                    </form>
                </div>
            </div>
